  ...
  const handleSelectResult = (result: any) => {
    ...
    if (result !== undefined) {
      ...
      const newRegion = {
        ...
        longitudeDelta: result.geometry?.location.lng / 100,
      };
    ...
    <GooglePlacesAutocomplete
      ...
      onPress={(data, details = null) => {
        if (data?.geometry == undefined) {
          ...
          axios.get(`https://maps.googleapis.com/maps/api/place/details/json?placeid=${data.place_id}&key=${MAP_API_KEY}`)

            .then((json) => {
              console.log('fetch api', json.data.result);

              handleSelectResult(json.data.result)
            })
            .catch((error) => {
              console.error(error);
            });
