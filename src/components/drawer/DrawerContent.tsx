...
import { AsyncKeys, getItem, saveItem, removeItem } from '../../constants/helpers';
...
const DrawerContent: FC<ScreenProps> = ({ navigation }) => {
  ...
  const logOut = () => {
    setTotal(0)
    removeItem('totalVoucher')
    // removeItem('addressName')
    // removeItem('addressPosition')
  ...
  const getTotalVoucher = async () => {
    const res = await getItem('totalVoucher')
    if (res)
      setTotal(res)
    else {
      try {
        const data = await axiosAPI.get('user/get-user-vouchers');
        setTotal(data.data?.transaction.length)
        saveItem('totalVoucher', data.data?.transaction.length)
      } catch (error) {
        setTotal(0)
      }
    }
