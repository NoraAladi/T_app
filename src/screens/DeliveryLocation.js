      ...
      .then(json => {
        ...
        if (save) {
          if (
            ...
            AsyncStorage.setItem('addressName', addressComponent)
            AsyncStorage.setItem('addressPosition', JSON.stringify(region))
  ...
  return (
    <Container style={{ backgroundColor: Colors.sacandAppBackgroundColor }}>
      ...
      <MapView
        ...
        initialRegion={Platform.OS === 'android' ? userCurrentLocation : region}
        minZoomLevel={6}
      ...
      >
        <Marker
          onDragEnd={e => {
            ...
            console.log(x);
            //Platform.OS === 'android' ?
            mapView.current.animateToRegion(e.nativeEvent.coordinate, 500)// : null
            setRegion(x);
            setTimeout(() => {
              setMarker(x);
            }, 500);
          ...
          draggable
          ref={_marker}
          coordinate={Platform.OS === 'android' ? userCurrentLocation : marker}
        />
        {
          // <MapViewDirections
